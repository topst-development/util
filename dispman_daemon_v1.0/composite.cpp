...
int composite_send_hpd_status(void)
{
	int flag = 0;
    ...
    if (ioctl(composite_fd,TCC_COMPOSITE_IOCTL_HPD_SWITCH_STATUS, NULL) ) {
...
int composite_display_output_attach(char onoff, char lcdc)
{
	TCC_COMPOSITE_START_TYPE composite_start;
    	char value[PROPERTY_VALUE_MAX];
	static unsigned int before_cvbs_mode = 0;
	unsigned int composite_mode;
