...
int BlockHeadParse(stDBC* pPos, stCEABlock* pCEA, unsigned char* pHDROffset, int prtflag)
{
    ...
    switch(BlkHdr.B.BlockType)
    {
        ...
        case 3:
            ...
            memcpy(&gVSDB,        pPos, (  (BlkHdr.B.ByteNumber > sizeof(pCEA->VSDB)) ? sizeof(pCEA->VSDB)         : (BlkHdr.B.ByteNumber+1) ));
            // check if it is HDMI VSDB
            // if so, check identifier value, if it's hdmi vsbd - return offset
            //MSGPRT(prtflag,("\tIEEE Registration Number : 0x%2X%2X%2X\n", gVSDB.ieeeregid[2],gVSDB.ieeeregid[1],gVSDB.ieeeregid[0]));
            if(gVSDB.ieeeregid[0] == 0x03 && gVSDB.ieeeregid[1] == 0x0C && gVSDB.ieeeregid[2] == 0x00)
            {
		...
		MSGPRT(prtflag,("\tHDMI_VIC_LEN             : %d\n",  pCEA->VSDB.vic_3d_len.B.hdmi_vic_len));
			...
			MSGPRT(prtflag,("\t It have multiple VSDB!!!!! we don't use it!!!!\n"));
		}
...
unsigned char EDID_Sample_MultipleVSDB[]=
{
	0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x38,0xA3,0x98,0x79,0x01,0x01,0x01,0x01,
	0x0F,0x12,0x01,0x03,0x80,0x00,0x00,0x78,0x2A,0x2A,0x02,0xA9,0x54,0x5D,0x9D,0x25,
	0x0E,0x52,0x5D,0xBF,0xEE,0x80,0x61,0x59,0x71,0x59,0x81,0x19,0x81,0x40,0x81,0x80,
	...
	0x6E,0x03,0x0C,0x00,0x10,0x00,0x00,0x80,0x3C,0x21,0x80,0x80,0x01,0x02,0x04,0x6F,
...
static int ReadEDIDBlock(const unsigned int blockNum, unsigned char* outBuffer)
{
        ...
        //	memcpy(outBuffer, &EDID_Sample_MultipleVSDB[edid_block_num*128], 128); //if you want to test Multiple VSDB...
                            ...
                            const enum PixelAspectRatio pixelRatio)
{
    ...
    if ( videoFormat == v640x480p_60Hz // if it's 640x480p@60Hz
         && gEDID.ESTiming1.B.est640480a60 == 1 && pixelRatio == HDMI_PIXEL_RATIO_4_3) // it support
    {
         return 1;
    }
    ...
    {
        if( !(videoFormat == v640x480p_60Hz && pixelRatio == HDMI_PIXEL_RATIO_16_9)) {
                return 1;
        }
    }
...
int Check3DFormatSupport(void)
{
	...
	for ( edid_index = 1 ; edid_index <= gExtensions ; edid_index++ )
	{
			...
			&& ((StartAddr = GetVSDBOffset(edid_index)) > 0) )   // check block
		{
			...
			{
				VSDBHdmiVideoPre = gEdidData[StartAddr + EDID_HDMI_EXT_POS] & EDID_HDMI_VIDEO_PRESENT_MASK;
			...
			latency_offset = (gEdidData[StartAddr + EDID_HDMI_EXT_POS] & EDID_HDMI_LATENCY_MASK) >> EDID_HDMI_LATENCY_POS;
            if (latency_offset == 0)
            {
              	latency_offset = 4;
            }
			...
			// HDMI_VIC_LEN
			HDMIVICLen = (gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS] & EDID_HDMI_VSDB_VIC_LEN_MASK) >> EDID_HDMI_VSDB_VIC_LEN_BIT;

			// HDMI_3D_LEN
			HDMI3DLen = gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS] & EDID_HDMI_VSDB_3D_LEN_MASK;

			DPRINTF("gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS] = %x\n", gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS]);
			...
			{
				DPRINTF("gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS] = %x\n", gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS]);
				DPRINTF("gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS] = %x\n", gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS]);

				VSDB3DPresent = gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS] & EDID_HDMI_3D_PRESENT_MASK;
				VSDB3DMultiPresent = gEdidData[StartAddr + EDID_HDMI_3D_PRESENT_POS] & EDID_HDMI_3D_MULTI_PRESENT_MASK;
			}
			...
			if ( blockLength >= (EDID_HDMI_EXT_LENGTH_POS - latency_offset))
			{
				...
				if (HDMI3DExtLen)
				{
					...
					{
						VICOrder = gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS + HDMIVICLen + (VSDB3DMultiPresent>>EDID_HDMI_3D_MULTI_PRESENT_BIT)*2 + edid_index*2] & EDID_HDMI_2D_VIC_ORDER_MASK;
						VICOrder = (1<<VICOrder);
						Hdmi3DStructure = gEdidData[StartAddr + EDID_HDMI_EXT_LENGTH_POS + HDMIVICLen + (VSDB3DMultiPresent>>EDID_HDMI_3D_MULTI_PRESENT_BIT)*2 + edid_index *2] & EDID_HDMI_3D_STRUCTURE_MASK;
						Hdmi3DStructure = (1<<Hdmi3DStructure);

					} // for
				} // if HDMI3DExtLen
			} // if ( blockLength >= EDID_HDMI_EXT_LENGTH_POS )
 ...
 */
int EDID3DFormatSupport(const struct HDMIVideoParameter * const pVideo)
{
	int edid_index;
	unsigned int StartAddr;
	unsigned int vic;
...
int API_EDID_GetSize(void)
{

    unsigned char temp[SIZEOFEDIDBLOCK];
	int extensionblocknum = 0;
	int edidblocksize = 0;
	...
	if (!ReadEDIDBlock(0,temp))
	{
	 ...
	 // get extension
	extensionblocknum = temp[EDID_EXTENSION_NUMBER_POS];


    DPRINTF("EDID Extension blocks %d...\n", gExtensions);

    if(extensionblocknum>3)
    {
        DPRINTF("EDID Extension blocks can't be greater than 3...\n");
       extensionblocknum = 3;
    }
	...
	return edidblocksize = (extensionblocknum + 1)*SIZEOFEDIDBLOCK;

}


int API_EDID_Read(unsigned char *pBuffer)
{

    unsigned char temp[SIZEOFEDIDBLOCK];
	unsigned char *pEdidBuffer = 0;
	int extensionblocknum = 0;
	int edidblocksize = 0;
	...
	edidblocksize = (extensionblocknum + 1)*SIZEOFEDIDBLOCK;

    // prepare buffer
    pEdidBuffer = (unsigned char*)malloc(edidblocksize);

    // copy EDID Block 0
	memcpy(pEdidBuffer, temp, SIZEOFEDIDBLOCK);

    if (!pEdidBuffer)
        return 0;

	for ( blkid = 1,blkpos = SIZEOFEDIDBLOCK; blkid <= extensionblocknum; blkid++ ,blkpos+=SIZEOFEDIDBLOCK )
	 {
		 // read extension 1 ~ rest of extension
		 if (!ReadEDIDBlock(blkid, pEdidBuffer+blkpos))
		 {
			 //Read Fail
			 return 0;
		 }
	...
	memcpy(pBuffer, pEdidBuffer, edidblocksize);

	free(pEdidBuffer);

	EDIDClose();

	return edidblocksize;

}

int API_EDID_Get_SupportAI(void)
{
        int ai_support = 0;

        if(gVSDB.ieeeregid[0] == 0x03 && gVSDB.ieeeregid[1] == 0x0C && gVSDB.ieeeregid[2] == 0x00) {
                ai_support = gVSDB.colorsink.B.AIsupport;
        }
        DPRINTF("%s ai_support=%d\r\n", __func__, ai_support);
        return ai_support;
}
