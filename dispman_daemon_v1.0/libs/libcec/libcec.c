...
#if CEC_DEBUG
#define DPRINTF(args...)    ALOGE(args)
#else
#define DPRINTF(args...)
#endif
...
int CECStart(void)
{
    if (ioctl(fd, CEC_IOC_START, NULL)) {
        DPRINTF("ioctl(CEC_IOC_START) failed!\n");
        return 0;
    }
...
int CECStop(void)
{
    if (ioctl(fd, CEC_IOC_STOP, NULL)) {
        DPRINTF("ioctl(CEC_IOC_STOP) failed!\n");
        return 0;
    }
...
int CECGetReumeStatus(void)
{
	...
	if( ioctl(fd, CEC_IOC_GETRESUMESTATUS, &resume_status)) {
		DPRINTF("ioctl(CEC_IOC_GETRESUMESTATUS) failed!\n");
		return 0;
	}
...
int CECAllocLogicalAddress(int paddr, enum CECDeviceType devtype)
{
    ...
    if (fd == -1) {
        DPRINTF("open device first!\n");
        return 0;
    }

    if (CECSetLogicalAddr(laddr) < 0) {
        DPRINTF("CECSetLogicalAddr() failed!\n");
        return 0;
    }
    ...
    if (laddr == CEC_LADDR_UNREGISTERED) {
        DPRINTF("All LA addresses in use!!!\n");
        return CEC_LADDR_UNREGISTERED;
    }
...
int CECSendMessage(unsigned char *buffer, int size)
{
    ...
    if (size > CEC_MAX_FRAME_SIZE) {
        DPRINTF("size should not exceed %d\n", CEC_MAX_FRAME_SIZE);
        return 0;
    }

#if CEC_DEBUG
    DPRINTF("CECSendMessage() : ");
...
int CECReceiveMessage(unsigned char *buffer, int size, long timeout)
{
	...
	if( bytes > 0)
	{
	    DPRINTF("CECReceiveMessage() : ");
	    CECPrintFrame(buffer, bytes);
	}

	if(bytes < 0)
	{
		DPRINTF("CECReceiveMessage() Error is occurred!!!!!! %d", bytes);
	}
#endif
...
int CECSetLogicalAddr(unsigned int laddr)
{
    if (ioctl(fd, CEC_IOC_SETLADDR, &laddr)) {
        DPRINTF("ioctl(CEC_IOC_SETLA) failed!\n");
        return 0;
    }
...
{
	DPRINTF("CECSendReceiveMessage : %d\n", uiData);
...
int CECCheckPhysicalAddress(int paddr )
{
    ...
    if (!EDIDGetCECPhysicalAddress(&paddr_edid)) {
        DPRINTF("Error: EDIDGetCECPhysicalAddress() failed.\n");
    }
