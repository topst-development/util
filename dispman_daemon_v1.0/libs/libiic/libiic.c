 ...
 */
static int iic_fd = -1;
static int iic_i2c_write_ioctl = I2C_SLAVE;

#define DISPMAN_SYSFS "/sys/class/tcc_dispman/tcc_dispman"

static int IIC_GetStbMode()
{
	int read_count, sysfs_fd ;
	char tmpstr[512] = {0};
	sprintf(tmpstr, "%s/%s", DISPMAN_SYSFS, "tcc_output_mode_stb");
	sysfs_fd = open(tmpstr, O_RDWR);
	read_count = read (sysfs_fd, tmpstr, PROPERTY_VALUE_MAX);
	tmpstr[read_count] = '\0';
	close(sysfs_fd);
...
int IICOpen()
{
	...
	if(IIC_GetStbMode()) {
		iic_i2c_write_ioctl = I2C_SLAVE_FORCE;
	}
	else {
		iic_i2c_write_ioctl = I2C_SLAVE;
	}
...
//????????
	...
	retval = ioctl(iic_fd, iic_i2c_write_ioctl, devAddr>>1);
