...
function ussage() {
cat <<EOT
Ussage: mkspalsh pagesize img_num imgfile1 imgfile2 .... outfile
...
echo "source image format is BMP"

FMTSIZE=`./bmpbpp ${IMG_ORI[$idx]}`


if [ "$FMTSIZE" -eq "24" ];  then
...
else if [ "${FILE_FMT[idx]}" = "JPEG" ]; then
...
MK_SPLASH="./mksplash $PAGESIZE $NIMG ${IMG_FN[*]} ${IMG_RSL[*]} ${FILE_FMT[*]} $FILENAME"
echo $MK_SPLASH
$MK_SPLASH
