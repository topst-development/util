...
int main(int argc, char *argv[])
{
	...
	image_fmt = image_resolution+ImageCnt;
	...
	for( idx = 0; idx < ImageCnt ; idx++)
	{
		...
		if((splash_buffer[idx].data = load_file(image_name[idx] , &size))){
			...
			memcpy(splash_image.SPLASH_IMAGE[idx].ucFmt, image_fmt[idx], strlen(image_fmt[idx]));
			...
			if(PAGE_ALIGN(size, page_size) != size) {
				splash_image.SPLASH_IMAGE[idx].Padding = PAGE_ALIGN(size, page_size) - size;
			}
			else {
			...
			printf(" ### %d  \n",splash_image.SPLASH_IMAGE[idx].ulImageAddr);
			printf(" ### %d  \n",splash_image.SPLASH_IMAGE[idx].ulImageSize);
			...
			printf(" ### %s  \n",splash_image.SPLASH_IMAGE[idx].ucFmt);
		}
